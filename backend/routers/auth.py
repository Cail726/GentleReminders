...
from dependencies import get_current_user, check_login_rate, json_error, json_ok, verify_csrf
...
def register(
    ...
    if not username or len(username) < 2 or len(username) > 30:
        return json_error("用户名需2-30个字符")
    if not password or len(password) < 6:
        return json_error("密码需至少6位")
...
def login(
    ...
    return json_ok(msg="登录成功", user_id=user.id)
